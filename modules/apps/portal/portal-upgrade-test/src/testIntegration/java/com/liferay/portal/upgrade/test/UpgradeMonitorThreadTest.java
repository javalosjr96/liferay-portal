...
public class UpgradeMonitorThreadTest {
	...
	public void testGetActiveQueries_DetectsDatabaseLocks() throws Exception {
		...
		for (DB.RunningQuery q : activeQueries) {
			if ((q.getQuery() != null) &&
				q.getQuery(
				).contains(
					"active"
				...
				Assert.assertTrue(q.isLocked());

				Assert.assertNotNull(q.getSchema());
				Assert.assertFalse(
					q.getSchema(
					).isEmpty());

				Assert.assertTrue(q.getDuration() >= 0);
