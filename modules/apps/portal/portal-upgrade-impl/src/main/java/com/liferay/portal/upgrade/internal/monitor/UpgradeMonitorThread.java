...
package com.liferay.portal.upgrade.internal.monitor;
 ...
 * @author Jorge Avalos
 */
public class UpgradeMonitorThread extends Thread {

	public UpgradeMonitorThread(DB db) {
		super("Upgrade-Monitor-Thread");
				...
				if (_log.isWarnEnabled()) {
					_log.warn(
						StringBundler.concat(
							...
							"Monitoring will be disabled for this remaining ",
							"upgrade execution. Please check database ",
							"resource usage.", sqlTimeoutException));
				}
				...
				if (_log.isWarnEnabled()) {
					_log.warn(
						StringBundler.concat(
							...
							"for this remaining upgrade execution. Please ",
							"check database permissions for execution of: ",
							_db.getPollerQuery()),
						exception);
				}
	...
	private static final long _POLLING_INTERVAL = 60000;

	private static final Log _log = LogFactoryUtil.getLog(
		UpgradeMonitorThread.class);
